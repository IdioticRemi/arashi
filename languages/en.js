...
module.exports = class extends Language {
	...
	constructor(...args) {
		...
		this.language = {
			...
			REQUIRES_EMOJI_PERMISSION: "I'm sorry sir but in order to work properly I need the permission to **Use Custom Emojis**!",
			...
			COMMAND_INFO: [
				"Hey what's up? I'm a cool Discord bot developped using the **discord.js** library!",
				"All credits to my developper: **- Ҝizuru キズル -#9535**.",
				"Also, big thanks to the **Klasa framework**'s developpers which I'm using to run.",
				"",
				"So, who am I? Well, just an another Discord bot! But I have some cool features for you!",
				`There is a ${vip} status! Which is not yet obtainable since there's not any economy.`,
				"This status gives you very cool perks! **More custom playlists**, **more custom prefixes**",
				"**Commands**, **boosts** and even **a role on the official support server**!",
				"There are lots of commands, check them out using the **help** command!",
				"A cool **experience system** and you even have a **very cool music system**.",
				"A very cool thing is... I am entirely **open-source**! Check my code here: **Kizuru/Arashi**",
				"",
				"Thanks to all my supporters! I love y'all! :blue_heart:"
			],
			...
			COMMAND_HELP_LOADING: `${gears} Generating Display...`,
			...
			COMMAND_HELP_INFOS: (display) => [
				`This is my help command!`,
				`You can take a look at my commands using reactions`,
				`under this message!`,
				``,
				`__**Usage**__:`,
				``,
				`${this.client.emojis.find(e => e.id === display.emojis.info)}: Jump to this page (Information page)`,
				`${this.client.emojis.find(e => e.id === display.emojis.first)}: Jump to the first page`,
				`${this.client.emojis.find(e => e.id === display.emojis.back)}: Jump to the previous page`,
				`${this.client.emojis.find(e => e.id === display.emojis.forward)}: Jump to the next page`,
				`${this.client.emojis.find(e => e.id === display.emojis.last)}: Jump to the last page`,
				`${this.client.emojis.find(e => e.id === display.emojis.stop)}: Remove all reactions to save internal memory`,
				`${this.client.emojis.find(e => e.id === display.emojis.jump)}: Jump to the \`x\` page`
			].join("\n"),
			...
			COMMAND_PLAY_ERRCON: `${no} I couldn't join the voice channel you're in.`,
			...
			COMMAND_BTC_DESCRIPTION: "Change the text channel where music information are sent.",
			COMMAND_BTC_NOVOICE: (channel) => `${no} You need to be inside the **${channel}** channel in order to bind a new text channel!`,
			COMMAND_BTC_SUCCESS: (channel) => `${ye} Successfully bound text channel to ${channel}!`,

			COMMAND_BVC_DESCRIPTION: "Change the voice channel where I play music.",
			COMMAND_BVC_NOPE: `${no} Sorry but couldn't find the voice channel!`,
			COMMAND_BVC_SUCCESS: (channel) => `${ye} Successfully bound voice channel to **${channel.name}**!`,
			...
			REQUIRES_VIP_STATUS: (tag) => `${no} I'm very sorry **${tag}** but you have to be a ${vip} member in order to execute this very cool command!`,
		};
	}
